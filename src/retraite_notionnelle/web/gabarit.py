...
:root {
  ...
  --fond-carte: #0f4a46;
  --fond-appui: #0f4a46;
  /* Le fond sur lequel un tableau défile : le voile qui cache l'ombre de
     défilement en bout de course doit être de cette couleur-là, et chaque
     carte le redéfinit à la sienne (voir `.defilant`). */
...
/* Le bandeau : une seule rangée, le nom du site à gauche, les onglets à
   droite, le tout sur le même vert que la page et fermé par un simple filet.
   Il est collé en haut parce que la navigation est une barre de neuf onglets :
   sur les pages longues — Coût, Données —, la reperdre au premier défilement
   obligeait à remonter de six écrans pour changer de page. Il ne porte plus
   de lien de retour vers le site du parti : le pied de page l'a, et le
   bandeau n'a pas à dire d'où l'on vient sur chacune des neuf pages. */
header.bandeau {
...
/* Le nom du site, en capitales serrées, précédé du carré d'or : c'est la
   ...
   synthèse vocale n'a à l'annoncer. Il a la hauteur de la rangée, comme les
   onglets, pour que la rangée n'ait qu'une hauteur : celle de la barre. */
header.bandeau .nom {
...
/* Dans le cadre que le site parent ouvre sur sa page d'accueil — même
   ...
   au-dessus, et la ligne de pied qui y renvoie ferait doublon. C'est la seule
   chose que la page sache de son hôte, et elle ne coûte rien s'il cesse de la
   poser. */
body.plf-embedded footer .retour-site { display: none; }
/* Les onglets. Ils ont remplacé les trois groupes étiquetés — « le programme,
   ...
   La marge négative d'un pixel pose le filet des onglets SUR le filet du
   bandeau : l'onglet courant s'y accroche, comme l'onglet d'un classeur. */
header.bandeau nav {
  display: flex; flex-wrap: wrap; gap: 0; margin-bottom: -1px;
...
/* Un onglet. Toute la hauteur de la barre — 56 px, bien plus que les 44 de la
   ...
   PAS QUE PAR LA COULEUR — un fond d'or seul ne se voit ni en niveaux de
   gris, ni pour une vision basse, et ne s'annonce pas ; il porte donc un
   soulignement de 2 px ET `aria-current="page"`. Les règles sont ancrées
   sur `header.bandeau` : le plan d'une page longue est un `<nav>` lui aussi,
   et un `nav a` nu lui donnait la hauteur des onglets. */
...
/* Sous 78 rem — le nom, les neuf onglets et leur jour font 1 168 px —, la
   rangée unique ne tient plus : le nom prend sa propre rangée, plus basse,
   et les onglets passent dessous, sur toute la largeur. Deux rangées de
   44 px : la barre reste collée sans manger l'écran. */
...
/* « En bref », en tête des résultats : ce que l'électeur retient s'il ne lit
   rien d'autre. L'encart d'une note, en plus grand, et les sommes en or : ce
   sont les seuls chiffres de la page qu'il doit emporter. */
.en-bref {
  border-left: 3px solid var(--or); background: var(--fond-carte);
  padding: 1rem 1.25rem; margin: 1.5rem 0; border-radius: 0 4px 4px 0;
  font-size: 1.125rem; line-height: 1.55; max-width: 46rem;
}
.en-bref .surtitre { margin: 0 0 0.75rem; font-size: 0.875rem; }
.en-bref p { margin: 0.6rem 0 0; }
.en-bref p.discret { margin-top: 0.9rem; font-size: 0.9rem; }
...
/* Le bas du formulaire : « Calculer », et « Effacer ma saisie » quand le
   navigateur en garde une. Côte à côte, et l'un sous l'autre sur un
   téléphone, sans que le second colle au premier. */
.envoi { display: flex; flex-wrap: wrap; gap: 0.75rem; align-items: center; }
/* Ce que le simulateur a retenu : une phrase à la place de la consigne,
   filetée d'or comme ce qui s'adresse au lecteur plutôt qu'à sa carrière. */
form .memoire {
  margin: 0.3rem 0 0; padding: 0.55rem 0.85rem; font-size: 0.92rem;
  border-left: 3px solid var(--or); background: var(--fond-appui);
}
...
/* La légende écrivait la couleur du texte doux SUR CRÈME, alors que le
   formulaire long est sur carte sombre : « PREMIER MÉTIER » y était du vert
   foncé sur vert foncé, et ne se lisait qu'à l'impression. */
.creme .metier > .rang { color: var(--sur-creme-doux); }
...
/* Le dépôt d'un relevé de carrière. L'encadré en pointillés dit ce qu'il est —
   une zone où l'on pose un fichier — avant qu'on ait lu la phrase, et le
   pointillé se ferme en trait plein quand un fichier passe au-dessus. Le champ
   de fichier reste NATIF : le remplacer par un bouton et un champ masqué coûte
   le nom du fichier choisi, que le navigateur affiche de lui-même, et met en
   jeu le clavier et les lecteurs d'écran pour un gain de dessin. */
.depot {
  border: 1px dashed var(--trait); border-radius: 4px;
  padding: 1rem 1.1rem; margin-top: 1rem; background: var(--fond-carte);
}
.depot.survol { border-style: solid; border-color: var(--or); }
.depot > :first-child { margin-top: 0; }
.depot > :last-child { margin-bottom: 0; }
.depot-appel { font-size: 0.98rem; }
.depot-bouton {
  display: flex; flex-wrap: wrap; align-items: center; gap: 0.5rem 0.75rem;
}
.depot-bouton label { font-size: 0.92rem; color: var(--texte-doux); }
.depot input[type="file"] { font: inherit; font-size: 0.88rem; max-width: 100%; }
/* Le compte rendu de la lecture : ce qui a été lu, ce qui a été supposé, ce
   qui n'a pas été compris. Vide, il ne prend pas de place — une bordure sans
   texte se lirait comme un message qu'on n'arrive pas à afficher. */
.rapport-releve:empty { display: none; }
.rapport-releve {
  border-left: 3px solid var(--reste); padding: 0.1rem 0 0.1rem 0.8rem;
  font-size: 0.92rem; margin: 1rem 0 0;
}
.rapport-releve.echec { border-left-color: var(--manque); }
.rapport-releve ul { margin: 0.4rem 0 0; padding-left: 1.1rem; }
.rapport-releve li { margin: 0.2rem 0; color: var(--texte-doux); }
.rapport-releve code {
...
.defilant {
  ...
  /* Conteneur de requête : la légende du tableau se dimensionne en `cqw`
     sur la zone visible, et non sur le tableau qui la déborde. */
...
/* Le voile prend la couleur de ce qui porte le tableau. Peint dans le vert de
   la page, il faisait sur chaque carte une bande d'un ton plus sombre au bord
   droit, même quand rien ne défilait. */
.carte, .note, .plan, .erreur, .cadre-carte, .engagements, section.cle {
  --fond-defilant: var(--fond-carte);
}
.creme { --fond-defilant: var(--creme); }
...
/* Une `<caption>` est large comme son tableau : quand il déborde de l'écran,
   elle est coupée avec lui (« Ce que le plancher individualisé change, p »).
   Elle porte donc un `<span>` collant, large comme la zone visible — `cqw`
   se mesure sur `.defilant` —, qui suit le défilement sans jamais le quitter.
   Une `<caption>` ne peut pas être collante elle-même, et en bloc elle passe
   sous l'en-tête : c'est le `<span>` qui l'est. */
caption > span {
  display: block; position: sticky; left: 0; width: 100cqw; box-sizing: border-box;
}
tbody th { font-weight: 700; }
/* Une ligne « dont » ventile celle du dessus : elle se lit en retrait et en
   maigre, comme dans le tableau du COR qu'elle imite, et une ligne « pour
   mémoire » de même — elle n'entre dans aucun total. */
tbody th .dont { font-weight: 400; color: var(--texte-doux); padding-left: 1.2rem; display: inline-block; }
...
/* `:is(th, td)`, et non `td` seul : l'en-tête de rangée gardait son filet, et
   chaque tableau finissait par un trait orphelin sous sa première cellule. */
tbody tr:last-child :is(th, td) { border-bottom: none; }
td.nombre, th.nombre { font-variant-numeric: tabular-nums; }
/* Un nombre ne se coupe pas : « 12 028,70 € par / an », « 62 / ans »,
   « 2026-09- / 14 » se lisaient ainsi dès qu'une colonne manquait de place. */
td.nombre, td.date, th.date { white-space: nowrap; }
/* Une colonne de NOTES, longues de plusieurs lignes : sans largeur minimale,
...
/* Les entrées du plan ont la composition des onglets d'avant la barre — elles
   l'héritaient d'un `nav a` nu, elles la portent maintenant en propre. */
.plan a {
...
/* Un montant par scénario, et un seul : le pouvoir d'achat de l'année de
   référence, PAR MOIS. Deux autres nombres l'ont accompagné puis quitté — la
   somme nominale du mois du départ, puis le total annuel. Une pension se pense
   au mois, comme un salaire ; le total annuel ne servait qu'à multiplier par
   douze sous les yeux du lecteur. */
...
/* Le revenu qu'une pension suppose : la réponse à la question posée, et donc
   le seul chiffre de la page qui vient AVANT les quatre montants. Il est écrit
   à la taille d'un montant de scénario — c'en est un, dans l'autre sens — et
   dans la couleur d'accent, qui dit qu'on le cherchait. */
.revenu-deduit .cle-chiffre {
  font-size: 1.875rem; font-weight: 900; letter-spacing: -0.03em;
  color: var(--accent); margin: 0.2rem 0 0.8rem;
}
...
.mot > .terme {
  ...
  text-transform: inherit; letter-spacing: inherit;
  /* Un bouton centre son texte : posé sur deux lignes dans une case étroite,
     le mot s'affichait centré sous un libellé aligné à gauche. */
  text-align: inherit;
}
...
/* La lecture d'une année. Elle S'ÉCRIT DANS LA LÉGENDE : chaque entrée porte
   une case, vide au repos, où la valeur de l'année survolée s'inscrit. Les
   cases ont d'avance la largeur de la plus longue valeur de leur série, posée
   par le script au rendu de la page : rien ne bouge donc au survol, ni quand
   le pointeur sort. Une bande à part — essayée deux fois — sautait : ses
   entrées se repliaient autrement à chaque année selon la largeur des
   chiffres, et remplacer la légende par la bande changeait la hauteur de la
   figure à chaque entrée et sortie du pointeur. L'année, elle, est écrite
   dans le tracé, en haut du trait de repère.

   La région `.lecture` reste : c'est ce que la synthèse vocale entend. Elle
   n'est plus affichée. */
.graphique .lecture {
...
.graphique .annee-lue {
  ...
  fill: var(--or); paint-order: stroke; stroke: var(--fond); stroke-width: 6px;
...
/* Un calcul SUR PLACE ne vide plus la page — c'est ce qui la faisait sauter
   en haut puis en bas à chaque clic : elle reste là, estompée, et ce trait
   glisse en haut de l'écran le temps du calcul, comme la jauge de l'écran
   d'attente. */
body.calcul-en-cours::after {
  content: ""; position: fixed; top: 0; left: 0; z-index: 30;
  width: 30%; height: 3px; background: var(--or);
  animation: glisse 1.4s ease-in-out infinite;
}
...
@media (max-width: 34rem) {
  body { font-size: 1rem; }
  /* Le bandeau ne colle plus : à neuf onglets sur deux ou trois rangées, il
     mangeait un tiers de la hauteur d'un téléphone, et c'est cette hauteur
  ...
  .scenario .chiffre .somme { white-space: nowrap; }
  ...
  /* Le SVG se réduisait avec la page, et ses textes, exprimés en unités du
     viewBox, avec lui : grossis à 24 unités sans que les marges du repère ne
     suivent, ils se chevauchaient — « k€ » sur « 1 500 », les décennies
...
@media (prefers-reduced-motion: reduce) {
  *, *::before, *::after {
  ...
  body.calcul-en-cours::after { width: 100%; }
}
...
#: ne charge rien de lui, et n'y renvoie que par ce lien, en pied de page,
#: pour qui a tout lu. Le bandeau n'y renvoie plus.
...
#: règles que celles qu'il vient de choisir dans le simulateur ; or ``lien``
#: est appelé à plus de cinquante endroits, au fond de corps de page qui ne
...
#: par rendu. Elle est donc posée en UN SEUL endroit — ``pages.rendre``, le
#: point d'entrée unique du rendu — et remise à zéro à chaque appel, de sorte
#: qu'aucun rendu ne peut hériter des réglages du précédent.
_OPTIONS = ""


def poser_options(requete: str) -> None:
    """Fixe les réglages que porteront les liens internes. Voir ``_OPTIONS``."""
    global _OPTIONS
    _OPTIONS = requete or ""
...
def lien(chemin: str, ancre: str = "") -> str:
    ...
    les trois pages agrégées calculent sous les règles qu'il a choisies, et
    non sous celles par défaut. Tant qu'il n'a rien changé, la requête est
    vide et l'adresse est celle d'avant, au caractère près.
    """
    return "#" + chemin + (f"?{_OPTIONS}" if _OPTIONS else "")


def route(chemin: str) -> str:
    """L'adresse NUE d'une page, sans les réglages : la cible d'un formulaire.

    Un formulaire en ``GET`` écrit lui-même la requête, à partir de ses
    champs ; le routeur d'``index.html`` colle celle-ci derrière l'action.
    Une action qui porterait déjà une requête en donnerait donc deux —
    « #/cout?indexation=prix?indexation=prix ». Les formulaires visent la
    route, les liens visent ``lien``.
    """
    return "#" + chemin
...
def entete(chemin_actif: str = "/") -> str:
    ...
    site, répété à l'identique sur neuf pages. Il reste un lien vers l'accueil,
    dans une ``<p>`` que le style compose en petites capitales.

    Le bandeau ne porte AUCUN lien vers le site parent : il l'a porté, en
    petites capitales au-dessus du nom, et c'était une rangée de plus sur
    chaque page pour dire d'où l'on vient. Le pied de page le dit, une fois.
    """
...
def tableau(entetes: list[str], lignes: list[list[str | Cellule]],
    ...
    # Le `<span>` est ce qui reste sous l'œil quand le tableau défile : voir
    # `caption > span` dans la feuille.
    legende = f"<caption><span>{escape(titre)}</span></caption>" if titre else ""
...
def mot(terme: str, definition: str) -> str:
    ...
    tabindex="0">`` : Chromium rend tout bouton en bloc en ligne, quoi qu'on
    lui dise, et un bloc ne coule pas dans une phrase — dans une case étroite,
    « vos 25 meilleures années, un taux » se brisait en quatre lignes autour du
    mot. Le rôle et la tabulation lui rendent ce que la balise donnait ;
    Entrée et Espace, c'est ``index.html`` qui les écoute. L'appel de bulle
    (:func:`bulle`), lui, reste un vrai bouton : c'est un pictogramme, pas un
    mot de la phrase.
    """
    return (
        f'<span class="mot"><span class="terme" role="button" tabindex="0" '
...
def graphique(titre: str, annees: tuple[int, ...], series: tuple[Serie, ...],
    ...
    if repere is not None and annees[0] <= repere <= annees[-1]:
        position = _abscisse(repere, annees[0], annees[-1])
        x = nombre_brut(position)
        # L'étiquette se pose du côté où il reste de la place : à droite du
        # trait dans la première moitié du tracé, à gauche dans la seconde,
        # faute de quoi « espérance de vie : 89,7 ans » sortait du cadre.
        a_gauche = repere > (annees[0] + annees[-1]) / 2
        etiquette = (
            f'<text class="graduation" x="{nombre_brut(position + (-5 if a_gauche else 5))}" '
            f'y="{nombre_brut(MARGE_HAUT + 8)}" text-anchor="{"end" if a_gauche else "start"}">'
...
def _legende(series: tuple[Serie, ...], libelle_ecart: str = "") -> str:
    ...
    devinés.
    """
    # `data-serie` et la case `.lu` : c'est là que le script écrit la valeur
    # de l'année survolée — voir `montrerLecture` dans index.html.
    entrees = "".join(
        f'<li data-serie="{rang}"><span class="pastille" style="background:{serie.couleur}"></span>'
        f"<span>{escape(serie.libelle)}"
        + (f' <span class="discret">{escape(serie.glose)}</span>' if serie.glose else "")
        + '</span><span class="lu"></span></li>'
