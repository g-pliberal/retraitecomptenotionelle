...
class Saisie:
    ...
    def verifier(self) -> None:
        ...
        if not ANNEE_MINIMALE <= self.annee_revenu <= ANNEE_MAXIMALE:
            raise ErreurSaisie(
                f"Année des montants saisis attendue entre {ANNEE_MINIMALE} et "
                f"{ANNEE_MAXIMALE} (reçu : {self.annee_revenu})."
            )
    ...
    def _verifier_unite(self) -> None:
        ...
        champ, et ceux-là paraissent quelle que soit l'unité choisie. Le
        contrôle vivait ici, derrière le retour anticipé ci-dessus, si bien
        qu'en multiples du salaire moyen « ?annee_revenu=9999 » passait et
        faisait afficher un SMIC de 10 932 € par mois.
        """
...
def _choix_unite(saisie: Saisie) -> str:
    ...
    return '<div class="grille">' + g.liste(
        ...
        "ci-dessous", type_="number", min=str(ANNEE_MINIMALE),
        max=str(ANNEE_MAXIMALE), step="1",
    ) + "</div>"
