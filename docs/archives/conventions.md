# Les conventions du dépôt, telles que `CLAUDE.md` les disait le 25 septembre 2026

*Archive, gelée.* Le fichier d'entrée des sessions, tel qu'il était avant
que la phase 1 de l'architecture ne le ramène à une page de règles
(`docs/architecture.md`, § 9.3 et annexe B). Il garde ce que la page ne dit
plus : l'histoire de chaque règle, et l'incident qui l'a fait naître — le
`main` local périmé du 18 septembre 2026, les branches `claude/*` auditées,
la réécriture de l'historique du 23 septembre, le compteur de commits non
poussés, les conflits sur les fichiers fabriqués. Les règles en vigueur sont
dans `CLAUDE.md` ; ce texte-ci n'en donne aucune. Ses versions antérieures
sont dans l'historique git de `CLAUDE.md`.
