# Feuille de route — les actions closes, et le journal

*Archive, gelée.* Ce fichier garde, à l'identique et dans leur ordre, les
actions closes de `docs/feuille_de_route.md` — faites, archivées ou
abandonnées — et son journal. La phase 1 de l'architecture les en a retirés
le 25 septembre 2026 (`docs/architecture.md`, § 9.3 et annexe B), pour que la
feuille de route ne porte plus que ce qui vit. Chaque action garde son
numéro : un renvoi « action N » du dépôt se lit ici quand l'action est close,
dans la feuille de route tant qu'elle est en cours. Rien ne s'y réécrit ; une
action qui se clôt vient s'y ajouter, à la fin. `python scripts/conservation.py`
vérifie que rien ne s'en perd.
