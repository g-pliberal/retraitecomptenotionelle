...
*Version 5.1 du projet, 25 septembre 2026, à valider avant d'entrer dans le
dépôt. Trois séries d'épreuves et une contre-épreuve l'ont façonnée (§ 14). La
5.1 a ajouté le tableau de bord et la règle « ce qu'on apprend n'est jamais
bloqué » (§ 9), sans toucher au noyau. La 5.2 ajoute la preuve et la façon de
trancher une divergence (§ 3.2 et 3.3) ; elle élargit aussi le principe 3 aux
autres modèles publics (§ 3.4), à la demande du propriétaire. La 5.3 garde
toutes les sources, précise leurs licences, et dit comment tirer le plus des
simulateurs officiels sans se faire bloquer (§ 3.4 et 3.5). La 5.4 en fait une
règle, sans jamais solliciter les caisses (§ 3.5). C'est la version décidée :
la phase 0 part d'elle.*

*Comment la lire.* Une session courte lit L'essentiel, le § 13 et l'annexe C.
Les § 1 et 14 et les commentaires des annexes sont des récits datés : en
entrant dans le dépôt, ils formeront la note de décision
`docs/decisions/0001-architecture.md`. Le reste décrit l'état : il reste vrai
tant qu'aucune décision ne le change.
...
*Récit du 25 septembre 2026, gelé à sa date.*

Le dépôt a été construit très vite : 654 commits en dix-sept jours, par 128
sessions. Chaque incident y a ajouté un contrôle, aucun n'a été retiré, et
aucun document ne répondait à la question « où va cette information ? ». Ce
jour-là :

- la suite complète de tests prenait 10 min 30 pour 2 501 tests, contre 35
  secondes pour 905 tests le 18 septembre ;
- tout le calcul du scénario 1 tenait dans une seule méthode,
  `ScenarioActuel.calculer`, de 1 494 lignes et 130 conditions, recopiée en
  JavaScript ;
- une règle vivait à une dizaine d'endroits : le rétablissement des
  fonctionnaires partis sans droit à pension (24 septembre) a modifié
  20 fichiers, dont 8 de prose ou de registres ;
- du 13 au 22 septembre, le dépôt s'était donné huit registres nouveaux
  (inventaire, réformes, pivots, veille, exemples officiels, affirmations du
  site, frontière contributive, zones de la prose) et plusieurs mécanismes de
  contrôle : presque un par jour, chacun avec son format et ses tests ;
- le registre de veille décrivait 98 règles, dont 12 seulement étaient citées
  dans le code, et deux registres se contredisaient sur une même règle
  (annexe A) ;
- la feuille de route pesait 1,1 Mo, aux deux tiers de journal : aucune
  session ne pouvait la lire en entier ;
- le modèle ne connaissait que le nombre des enfants, et les faisait tous
  naître quand l'assuré avait 30 ans.
...
dépôt ; celle de droite, ce que la couverture demande à l'architecture.

| Domaine | Aujourd'hui | Ce que la couverture demande |
|---|---|---|
| Régimes, droits propres | 91 lignes d'inventaire : 35 modélisées, 39 partielles, 15 hors champ, 2 routages | compléter les partielles ; ouvrir les hors champ (Alsace-Moselle, fonctionnaires de Mayotte, anciens régimes coloniaux, ORTF, Crédit foncier…) |
| Enfants | un nombre ; naissances présumées aux 30 ans de l'assuré | naissances et adoptions datées, et la date prévue d'une naissance ; qui élève l'enfant ; interruptions d'activité ; accord des parents, parents de même sexe, retrait de l'autorité parentale |
...
| Prélèvements sur les pensions | taux plein (9,1 %) pour tous | revenu fiscal du foyer, pour les exonérations et les taux réduits |
...
Les sources recensées le 25 septembre 2026 (§ 14.9), par usage :

| Usage | Source | Ce qu'elle apporte | Ce dont elle dépend |
|---|---|---|---|
| paramètres | les barèmes de l'IPP (Licence Ouverte), à jour de la loi du 30 décembre 2025 | 155 fichiers sur les retraites, dont une partie cite son texte | c'est la source des paramètres d'OpenFisca-France-Pension et de TRAJECTOiRE : un accord entre eux ne compte qu'une fois |
| indépendants | `modele-ti` de l'Urssaf (publicodes, MIT) | cotisations, trimestres, points de la CNAVPL, de la Cipav et du RCI ; environ une règle sur deux cite sa source | indépendant |
| pension entière | Destinie 2, de l'INSEE (GPL) | régime général, fonctionnaires de l'État et des collectivités, indépendants, Agirc-Arrco, réversion, minimum vieillesse ; droit jusqu'en 2023 | règles et paramètres pour l'essentiel indépendants de l'IPP |
| cas types, régimes spéciaux et libéraux | TRAJECTOiRE, de la DREES (EUPL) | un second code, en mode cas types | paramètres de l'IPP |
| pension entière, profils simples | OpenFisca-France-Pension (AGPL) | cinq familles, paramètres figés au 2 janvier 2025 | paramètres de l'IPP |
...
| les prélèvements sur les pensions | LexImpact, sur OpenFisca-France (AGPL) | la CSG, la CRDS et la CASA selon le revenu fiscal du foyer, là où le modèle applique 9,1 % à tous | OpenFisca-France |
...
montré dans les deux sens : sa règle d'âge code 62 ans et 9 mois pour la
génération 1967, quand le texte qu'elle recopie dit 63 ans et 9 mois. Le
modèle, lui, applique 63 ans et 6 mois, le droit depuis la loi du 30 décembre
2025.
...
- **OpenFisca-France-Pension** est une seconde implémentation, rejouée sur des
  profils simples dans les cinq familles qu'il couvre : régime général,
  pension civile (État et CNRACL), Arrco d'avant 2019, Agirc, Ircantec. Les
  confrontations ont trouvé cinq erreurs chez nous, corrigées, et sept chez
  lui, où le dépôt suit la loi.
...
- **Une présomption** remplace un fait inconnu. Elle a un nom, une valeur et
  ...
  n'est saisi :
  - enfants nés aux 30 ans de l'assuré ;
...
Une règle sur cinq, dans l'histoire du dépôt, dépend d'une autre. Ces liens
ont leur propre fiche, avec leurs versions. La priorité entre régimes pour les
trimestres d'enfants existait déjà dans la loi du 3 janvier 1975 (article 16),
et a eu six versions depuis 1985. L'ordre entre la surcote et le minimum
...
130 conditions, et le dépôt l'a appris à ses dépens : « l'ordre n'est pas
indifférent […], et le modèle en prenait deux à l'envers ».
...
La liste a déjà servi, et elle a montré ce que la mémoire laisse passer. Pour
écrire l'annexe A, on a comparé mot à mot, dans l'index LEGI, les rédactions
successives de trois articles :

- **L. 12 du code des pensions civiles et militaires** : huit rédactions dans
  l'index depuis 1982. Quatre touchent au b (1982, 2004, 2011, 2025) ; les
  quatre autres (2015, 2018, deux en 2023) le laissent identique.
- **R. 13 du même code** : six rédactions (1964, 2004, 2006, 2009, 2011,
  2025). Celles de 2006, 2009 et 2025 ne changent que des renvois.
- **L. 351-4 du code de la sécurité sociale** : quatorze rédactions depuis
  1985, dont celle d'origine. Sept changent le droit (2003, 2009, 2013, 2016,
  deux en 2023, décembre 2025). Quatre ne changent que des renvois (2010, deux
  en 2018, 2022). Deux sont à relire avec leur décret ou leur loi (2001,
  février 2025).

La version précédente de ce document portait, pour le régime général, une
fiche à trois versions. Celle de la fonction publique datait du 1er janvier
2011 une règle que la loi n'applique qu'aux pensions prenant effet à compter
du 1er juillet 2011.
...
Références mesurées le 25 septembre 2026, par carrière, une fois les données
chargées : 3 millisecondes pour le scénario 1 en Python et 0,7 en JavaScript ;
29 et 4,5 millisecondes pour les six scénarios.
...
- reliquider à des événements qui n'ouvrent aucun droit, vingt-cinq à
...
À la phase 2, il passe à la carte, sans changer de questions. Une maquette le
montre déjà (§ 14.7).
...
- **`docs/architecture.md`** : ce document, sans ses récits, avec son numéro de
  version et la liste de ses changements.
- **Les décisions** : `docs/decisions/`, une note courte par choix. La
  première, `0001-architecture.md`, est ce document tel qu'il a été décidé,
  gelé : pourquoi cette architecture, et comment elle a été éprouvée.
  `docs/architecture.md` en tire l'état, et c'est lui qui vit ensuite.
...
se lance seul. Les deux tests de trois minutes (les chiffres ancrés, les
témoins) sont découpés. Les témoins se découpent par domaine et par étape.
...
- Les identifiants ne se renomment jamais : on déprécie, on ne casse pas.
  OpenFisca-France en est à sa 176e version majeure — autant de changements
  qui cassaient ce qui s'appuyait sur lui —, et son journal parle de
  renommage plus d'une centaine de fois.
...
les 83 interrupteurs par période et les huit registres. De même, un registre
ou un mécanisme de contrôle n'entre dans le dépôt, et n'en sort, que par une
note.
...
## 14. Ce qui a été vérifié

*Récit du 25 septembre 2026, gelé à sa date. Trois séries d'épreuves : la
première a porté sur la version 1, la deuxième sur la version 3, la troisième
sur la version 4. Cette version-ci reprend les constats de la troisième, et ceux
d'une contre-épreuve faite sur elle-même (§ 14.6). Une maquette du tableau de
bord s'y est ajoutée (§ 14.7).*

### 14.1 L'histoire du dépôt

Trois cent dix-huit changements réels ont été rejoués sur la version 1 : les
44 bascules de `frontiere_contributive.yaml`, les 98 règles de `veille.yaml`,
les 109 réformes de `reformes.yaml` et les 67 commits du moteur du scénario 1.

- 249 (78 %) y entraient tels quels, par les points d'extension prévus.
- 56 demandaient un concept qu'elle ne nommait pas : la face et le payeur, le
  droit supposé du passé, les transformations de droits, le droit
  conditionnel, les pensions par fractions, les unités, le capital, les
  composantes temporaires, les versions inapplicables.
- 9 cassaient sa structure :
  - des règles qui ont besoin d'un résultat calculé plus loin (la priorité
    des enfants, le rétablissement) ;
  - un départ présumé « au taux plein », qui demande le moteur pour être
    connu ;
  - des scénarios rétroactifs qui ne commencent pas à la bascule.
- Une règle sur cinq dépendait d'une autre : plafonds, exceptions, priorités,
  choix, cumuls, séquences, ordres de composition.

**Deuxième série, sur la version 3.** Chaque constat a été repris un par un :
18 étaient absorbés, 5 l'étaient en partie, 1 ne l'était pas (les pensions
par fractions). Un échantillon tiré au hasard de 48 changements (graine
20260925) a été rejoué pour chercher une régression. 37 entraient aussi bien
qu'en version 1, et 9 mieux. Un entrait moins bien : une borne sur la date de
la période elle-même, que la liste fermée des dates avait perdue. Un
n'entrait toujours pas : la même pension par fractions.

**Troisième série, sur la version 4.**

- Les 219 constats des cinq rapports précédents ont été regroupés en 59 : 48
  étaient résolus, 11 en partie, aucun ne restait ouvert.
- Un nouvel échantillon de 40 changements (graine 31415), tiré hors des 48
  déjà rejoués, n'a montré aucune régression. L'un d'eux, un commit « Add
  files via upload », n'est pas un changement de droit. Des 39 autres, face à
  la version 1, 10 entraient mieux et 29 aussi bien ; face à la version 3,
  12 mieux et 27 aussi bien.
- La même relecture a relevé quinze contradictions internes, dont cinq
  touchaient le noyau ou son gabarit. La plus lourde : la règle
  d'information était au noyau, mais aucun contrat ne portait la date de
  l'événement qu'elle exige.

### 14.2 Des cas de droit parmi les plus difficiles

**Première série.** Vingt-huit cas : réversions entre ex-conjoints, décès
avant la retraite, carrières européennes, retraite progressive, cumul
emploi-retraite et seconde pension, départs multiples, invalidité, handicap et
pénibilité, droits maintenus des parents de trois enfants, minimum
contributif et son écrêtement, malus temporaire, revalorisation selon le total
des pensions, rachats, partage des trimestres d'éducation, conversions de
points, monnaies, droit futur, bascule, outre-mer, population. Sur la
version 1 : 1 cas tenait tel quel, 20 tenaient avec un amendement,
7 ne tenaient pas. Six échecs venaient de l'ordre linéaire des étapes, le
septième de la définition de la proposition. D'où l'échéancier, la liquidation
en fonction pure et fictive, le réseau de personnes et les univers de droit.

**Deuxième série, sur la version 3.** 21 cas tenaient, 6 en partie, 1 non (la
revalorisation calculée sur le total de toutes les pensions, droits dérivés
compris). Quatorze situations nouvelles ont été construites pour la casser :
une survivante polypensionnée en retraite progressive, une date calculée sous
l'ancienne version de sa propre règle, une loi rétroactive observée à deux
dates, deux couches qui touchent la même fiche, une chaîne de remariages sans
fin… Aucune ne demandait de principe nouveau. Toutes se ramenaient au modèle
d'exécution : l'état comme entrée de `liquider`, la nature d'une liquidation,
les étapes qui font vivre la pension sorties de `liquider`, les événements
induits ou publiés et leur ordre dans une même journée, l'empilement des
couches, une borne sur le réseau de personnes et sur l'emboîtement des
hypothèses.

**Troisième série : huit cas joués.** Huit situations ont été encodées dans
les contrats de la version 4. Quatre ont tourné de bout en bout dans une
maquette d'environ 1 700 lignes, avec une `liquider` pure, un journal où l'on
ne fait qu'ajouter et la règle d'information vérifiée :

- une mère fonctionnaire passée par le privé, deux enfants de part et d'autre
  de 2004 ;
- une réversion partagée entre deux ex-épouses, puis l'ASPA de l'une ;
- la revalorisation de 2020 sur le total de toutes les pensions ;
- la bascule du scénario 3 pendant une retraite progressive.

Les quatre autres ont été tracées sur papier :

- une retraite progressive, puis la pension définitive, puis une seconde
  pension ;
- une carrière européenne, avec l'écrêtement du minimum contributif ;
- des TUC validés après coup, observés à deux dates ;
- une invalidité remplacée par la pension pour inaptitude.

Aucune ne demandait de principe nouveau. Mais la maquette a dû prendre 34
décisions que le texte ne prenait pas, dont 21 dans les contrats. Elle a
aussi montré trois défauts de fond :

- **Le relevé dépend de la demande**, puisque des fiches d'acquisition lisent
  la date d'effet : il ne pouvait pas être une entrée de `liquider`.
- **« L'état de la veille » était faux.** Les étapes qui ne liquident rien le
  lisaient. Dans la maquette, l'ASPA d'une ex-épouse restait calculée sans la
  réversion liquidée le même jour, jusqu'à l'événement suivant, sept mois plus
  tard.
- **Le partage des versions ne tenait pas à toutes les dates d'observation.**
  Filtrées à une date d'observation, les versions de l'annexe A laissaient des
  trous : une version close par sa suivante ne l'est pas tant que la suivante
  n'est pas publiée.

Ces constats ont produit les § 4.1 (une borne ne vaut qu'une fois connue la
version qui la ferme), 5.5, 6.4 et 7.2 à 7.5. Ils
ont aussi changé les contrats de l'annexe C : la fiche en est un, le journal a
deux dates, et la liquidation porte l'événement qui l'appelle.

### 14.3 Dix ans de changements

**Troisième série.** Vingt-trois changements plausibles d'ici 2036 ont été
confrontés à la version 4 : un système universel par points, un âge pivot, un
pilier de capitalisation, des fusions de régimes, une réversion harmonisée, un
partage des droits entre conjoints, une coordination européenne révisée, une
décision de justice rétroactive, un coefficient d'équilibre appliqué, une
microsimulation de toute la population, une API publique, un troisième
langage, un OpenFisca arrêté, des contributeurs humains, d'autres modèles
d'IA… Quinze entraient sans toucher au noyau, cinq le touchaient, trois
restaient incertains. Les huit tenaient à six causes, toutes réparables sans
code :

- des listes de valeurs fermées dans le noyau, que chaque domaine aurait
  rouvertes ;
- une couche qui ne savait ni ajouter une règle ni viser un ensemble de
  règles ;
- des contrats absents : la fiche, la table datée, la surface publique ;
- un projet externe nommé dans un principe ;
- un modèle du temps qui se contredisait ;
- une frontière mal écrite entre le moteur, le pilote et le site.

La même revue a posé à « L'essentiel » et à « Où va quoi » cinq questions de
nouveau venu. Deux seulement y trouvaient leur réponse.

Ces constats ont produit :

- la règle additive (§ 13.3) ;
- les quatre sortes de dates (§ 4.2) ;
- les couches qui ajoutent (§ 4.8) ;
- les neuf contrats (§ 13.1) ;
- les principes 3 et 4 réécrits ;
- l'observation fixe ou glissante (§ 4.9) ;
- l'empreinte de la carte (§ 13.3) ;
- le pilote des paramètres de population (§ 7.7) ;
- un « Où va quoi » complété.

### 14.4 Les systèmes qui durent

La version 1 a été comparée à OpenFisca (dont OpenFisca-France-Pension), à
Catala, et aux modèles de retraite de l'INSEE (Destinie), de l'IPP (PENSIPP)
et de la DREES (TRAJECTOiRE), sur leur documentation et leur code publics.

- **Ce qui concorde** : les barèmes datés et sourcés, la version supposée
  (OpenFisca a ses valeurs « attendues »), les étapes aux entrées et sorties
  explicites, le texte d'abord, la confrontation à d'autres modèles, le calcul
  qu'on peut relancer pour retenir le plus favorable.
- **Où la proposition va plus loin** : la date qui décide, que ces systèmes
  laissent implicite. Destinie a dû corriger en 2011 une règle lue sur la
  génération au lieu de l'année d'ouverture des droits.
- **Ce qu'elle leur a pris** :
  - les conflits entre versions traités comme des erreurs (Catala) ;
  - l'axe de ce qu'on savait (Destinie, PENSIPP) ;
  - les scénarios en couches sur un droit de référence intact (OpenFisca).
    Une réforme d'OpenFisca peut ajouter, remplacer, mettre à jour ou
    neutraliser une variable, vérifié dans le code d'`openfisca-core` ;
  - des identifiants qui ne se renomment pas et le journal obligatoire
    (OpenFisca) ;
  - la population séparée des droits (Destinie, TRAJECTOiRE) ;
  - la monnaie de chaque montant.
- **Où elle s'en écarte exprès** : les proches, en liens datés de personne à
  personne plutôt qu'en entités fixes.

### 14.5 Des contrôles faits à la main

- **Le partage des versions se vérifie à chaque date d'observation.** Un
  programme d'une centaine de lignes lit les fiches de l'annexe A telles
  qu'elles sont écrites.
  - Fonction publique : sept versions, six dates d'observation, aucun trou,
    aucun chevauchement, six cas de bascule fabriqués seuls.
  - Régime général : six versions, cinq dates d'observation, aucun trou,
    aucun chevauchement, cinq cas de bascule.
  - Si l'on fait valoir chaque borne dès l'origine, comme la version 4, quatre
    des six dates d'observation de la première fiche laissent des trous, et
    trois des cinq de la seconde. C'est le défaut que la maquette avait
    trouvé.

  Sur les versions précédentes de la fiche, le même contrôle avait détecté
  deux erreurs glissées exprès, et un vrai trou que la rédaction avait laissé
  sans le voir : un enfant né au premier trimestre 2010, pour une pension
  prenant effet avant le 1er avril 2010.
- **Les textes, lus mot à mot.** Les rédactions successives de L. 12, R. 13
  et L. 351-4 ont été comparées dans l'index LEGI (§ 6.6). Elles ont corrigé
  la fiche de la fonction publique : la version de 2011 se coupe au 1er
  juillet. Elles ont aussi corrigé celle du régime général : une version de
  2003, dont le décret (D. 351-1-7) accordait les trimestres un par un, à
  chaque anniversaire de l'enfant. Et elles ont ouvert une question : l'index
  date du 1er juillet 1974 la loi du 3 janvier 1975.
- **Le moteur a de la marge** : 3 millisecondes par carrière pour le
  scénario 1 en Python, 0,7 en JavaScript. Un échéancier qui ne reliquide
  qu'aux événements qui ouvrent un droit coûte une à cinq fois plus. Un
  échéancier qui reliquiderait à chaque revalorisation coûterait vingt-cinq à
  cinquante fois plus : c'est pourquoi le § 7.4 l'interdit.

### 14.6 La contre-épreuve de la version 5

Les trois mêmes relectures ont repris la version 5, chacune sur ses propres
constats.

- **La revue à dix ans** tient le noyau pour stable dix ans. Ses dix
  propositions sont reprises, trois sous une autre forme qu'elle juge
  meilleure ou équivalente : neuf contrats au lieu de huit, quatre sortes de
  dates au lieu de quatorze, un statut « lue ou supposée ». Ses six
  contradictions sont levées. Les cinq questions d'un nouveau venu trouvent
  leur réponse dans « Où va quoi », contre deux avant. Elle a relevé trois
  contradictions de rédaction, corrigées depuis :
  - les empreintes sont gardées hors de ce qu'on compare au bit près ;
  - ce qu'une règle inscrit va au journal, pas dans la chronologie ;
  - le principe 3 s'écarte d'OpenFisca sur la législation telle que la caisse
    l'applique.
- **La reprise des constats** trouve réglées 14 contradictions sur 15, 18
  modifications sur 19 et 10 lignes partielles sur 11. Là où la version 5 a
  choisi une autre voie que celle proposée, cette voie tient. Les 87 renvois
  menaient à la bonne section, et le contrôle du partage passait. Elle a
  relevé deux phrases à corriger, et quatre écarts de vocabulaire, corrigés
  depuis :
  - le relevé s'arrête à la date d'effet ;
  - une version est connue à la publication du texte qui la fait naître.
- **La maquette** a rejoué les trois cas qui avaient le plus révélé, et tous
  tournent. L'ASPA voit maintenant la réversion liquidée le même jour. Les
  TUC ne comptent qu'une fois leur décret publié. Elle a trouvé réglées 25
  inventions sur 34 et 11 contradictions sur 12. Il restait 17 décisions à
  prendre, dont 8 dans le noyau, et 6 contradictions. Elles sont écrites dans
  cette version (§ 4.1, 4.8, 5.1, 6.3, 7.3, 7.4 et annexe C) :
  - la date à laquelle un fait est connu ;
  - la lignée d'une composante, et son extinction ;
  - la nature d'une révision ;
  - la place d'une couche qui ne vaut que pour un calcul ;
  - l'étape où se fait la valorisation ;
  - la date de publication d'une version ;
  - la résolution de l'univers quand l'observation glisse.

Une dernière vérification, étroite, a repris ces seuls points sur le texte
corrigé, et rejoué une troisième fois les trois cas. Tous tournent, et tout
était réglé, sauf quatre phrases qui ne touchaient aucun champ. Elles sont
écrites depuis, mais n'ont pas été rejouées :

- l'observation filtre les versions au calcul, non à la fabrication ;
- chaque transformation s'applique à l'étape qui produit les lignes qu'elle
  transforme ;
- une borne vaut dès que l'une des versions qui commencent là est connue ;
- une version sans texte est connue à toute date.

Trois points plus petits ont suivi, écrits de même : les revalorisations
qu'une révision pour l'avenir reprend, ce que la bascule garde ou retire, et
la fraction d'une retraite progressive. Il ne reste qu'une question, qui
n'est pas d'architecture : ce que la proposition fait d'une retraite
progressive en cours à la bascule. Le test des fiches sans décision (§ 4.8) a
d'ailleurs servi dans la maquette. La fiche de L. 351-16, qui remplace la
fraction par la pension complète, passait sans décision dans le scénario 3.

### 14.7 Une maquette du tableau de bord

*Ajoutée le même jour, à la demande du propriétaire : dire l'avancement, les
limites et ce qui reste à faire.*

Un script d'environ deux cent trente lignes l'a fabriquée depuis les registres
d'aujourd'hui, sans rien écrire dans le dépôt. Ce qu'elle dit déjà :

- **Les régimes.** 91 inventoriés : 35 modélisés, 39 partiels, 15 hors champ,
  2 routages. Pesés par leurs retraités (EACR 2024), 88 % des
  retraités-caisses relèvent d'un régime modélisé, et 11 % d'un régime
  partiel. Le plus peuplé des partiels est celui des fonctionnaires de l'État,
  avec 2,0 millions de retraités : ses bonifications de service ne sont pas
  servies.
- **Les règles.** Sur 98 suivies en veille, 44 sont conformes, 21 transcrites,
  24 approchées, 5 hors modèle, 3 manquantes, 1 à vérifier. 22 seulement sont
  confrontées à un exemple officiel, et 12 citées dans le code.
- **Hors du modèle**, la réversion pèse 10,4 % de la masse des prestations en
  2024.
- **Ce qui reste à faire était ailleurs.** La feuille de route compte 130
  actions, dont 123 faites et 5 en cours. 114 sources sur 260 restent à
  explorer, dont 9 visent un régime partiel.

Elle a aussi montré deux choses que les registres cachaient :

- **un blocage** : un exemple officiel que le modèle ne reproduit pas ne peut
  pas entrer, puisque le test exige que tous passent (§ 9.2) ;
- **un état peut-être périmé** : pour 17 des 24 règles approchées, l'effet
  raconte à l'imparfait l'erreur corrigée, sans dire ce qui reste.

### 14.8 Ce que coûte le travail aujourd'hui, et ce qui est déjà prouvé

*Mesures du 25 septembre 2026, faites sur l'historique git et les fichiers du
dépôt.*

- **Une modification coûte cher.** Sur les 400 derniers commits, un commit
  touche 7 fichiers en médiane. Un commit qui change le moteur du scénario 1
  en touche 20, dont 7,5 de prose ou de registres écrits à la main en
  moyenne.
- **Quatre fichiers pèsent sur tout.** La feuille de route est modifiée dans
  72 % des commits, `limites.md` dans 62 %. Avec `veille.yaml` et le
  `README.md`, que presque tout changement du moteur touche aussi, ils pèsent
  2,4 Mo : aucune session ne peut les lire en entier.
- **Les tests prennent 10 min 31** pour 2 501 tests.
- **Les données sont déjà prouvées.** 112 séries sont certifiées par des
  scripts qui les confrontent à leur source (dernier passage le 24 septembre
  2026) : 42 370 valeurs, dont 3 corrigées. Les phrases fortes du site sont
  confrontées au modèle, et celles qu'il contredit restent déclarées telles
  jusqu'à ce qu'on les corrige. Ce qui manque, c'est la même preuve pour les
  règles : 12 des 98 règles de la veille sont reliées au code.

### 14.9 Les autres modèles, recensés

*Recherche du 25 septembre 2026. Chaque affirmation vient d'une page lue ce
jour-là ; le code public de Destinie 2 et de TRAJECTOiRE, et la licence de
`modele-ti`, ont été revérifiés à part.*

- **Des confirmations vraiment indépendantes existent.** Destinie 2, de
  l'INSEE, publié sous GPL, calcule des pensions entières : régime général,
  fonctionnaires, indépendants, Agirc-Arrco, réversion, minimum vieillesse.
  Ses règles et ses paramètres ne viennent pas, pour l'essentiel, de l'IPP.
  S'y ajoutent `modele-ti` de l'Urssaf pour les indépendants, les agrégats de
  la DREES et les simulateurs officiels anonymes.
- **Beaucoup d'accords ne sont qu'apparents.** Les paramètres
  d'OpenFisca-France-Pension et de TRAJECTOiRE viennent des barèmes de l'IPP,
  et la DREES recalcule les cas types du COR avec TRAJECTOiRE. Leur accord ne
  vaut qu'une confirmation.
- **Tous peuvent se tromper.** Le dépôt a déjà trouvé les barèmes de l'IPP en
  retard : leur taux des chirurgiens-dentistes s'arrête en 2019, quand la
  caisse l'a changé en 2023. Il a trouvé sept erreurs chez
  OpenFisca-France-Pension. Seuls les barèmes de l'IPP et le simulateur
  officiel de l'assuré connecté intègrent la loi du 30 décembre 2025.
- **Deux contraintes.** Un code sous GPL, EUPL ou AGPL ne se copie pas dans le
  dépôt : on l'exécute à part, et on ne garde que ses sorties. Les simulateurs
  officiels interdisent l'extraction en masse : on n'en tire que des exemples
  ponctuels et cités.
- **Une erreur du dépôt.** La feuille de route dit encore que TRAJECTOiRE et
  Destinie « ne sont pas publiés » : leur code est public.
- **Le propriétaire garde toutes les sources**, Catala, PENSIPP et LexImpact
  compris. La même recherche a lu leurs licences dans leurs fichiers :
  - Apache 2.0 pour Catala ;
  - aucune licence pour PENSIPP, que GitHub laisse lire, pas reproduire ;
  - AGPL pour LexImpact et OpenFisca ;
  - GPL pour Destinie 2, EUPL pour TRAJECTOiRE ;
  - Licence Ouverte pour les barèmes de l'IPP, MIT pour `modele-ti`.

  La règle d'âge de Catala pour la génération 1967 a été relue dans son
  fichier : le code y dit 62 ans et 9 mois, le texte recopié juste au-dessus
  63 ans et 9 mois.
- **Les simulateurs officiels.** Leurs conditions d'utilisation ont été lues
  une à une. Aucune n'autorise l'extraction en masse, et aucun n'offre d'API
  ouverte : la seule API « retraite » recensée sur data.gouv.fr, celle de PRO
  BTP, est réservée aux administrations. En revanche :
  - l'Assurance retraite publie 19 jeux de données sous Licence Ouverte ;
  - le service des retraites de l'État publie ses statistiques sur
    data.gouv.fr ;
  - les fiches « Traitements algorithmiques » de l'Assurance retraite (mars
    2026) décrivent les traitements, sans leurs formules.

  Le droit d'accès aux documents administratifs aurait permis de demander
  davantage. Le propriétaire a choisi de ne pas solliciter les caisses
  (§ 3.5).

### 14.10 Les risques qui restent

| Risque | Ce qui le contient | Ce qui déclencherait une décision |
|---|---|---|
| Deux moteurs à tenir à la main | même structure, données partagées, témoins par étape ; une étape peut passer seule à une source unique (§ 7.1) | le coût de la recopie dépasse celui de la règle elle-même |
| L'ampleur du programme « tout couvrir » | des phases qui ne changent aucun résultat, puis un domaine à la fois, sur un même gabarit | un domaine qui n'entre pas dans le noyau (§ 13.4) |
| L'usure du noyau par les ajouts | la règle additive ; des listes hors du noyau ; quatre sortes de dates au lieu d'une liste de dates | un domaine qui demande une cinquième sorte de date |
| Une règle du droit réel qui fuit dans la proposition | les sélecteurs des couches ; la liste des fiches sans décision (§ 4.8) | une fiche appliquée dans un univers de la proposition sans décision |
| La frontière entre moteur, pilote et site | le pilote porte les comportements, les populations et les paramètres de population (§ 7.7) | un paramètre de population calculé par le moteur |
| Le temps de calcul, sur téléphone | budget mesuré à chaque phase ; paquet chargé par domaine | un budget dépassé |
| Le droit à venir | les versions supposées, en couches, déclarées dans le résultat | une loi nouvelle |
| Un autre modèle en retard, abandonné, ou qui dépend d'un autre | il n'est qu'une confirmation, jamais une dépendance ; le registre dit de quoi il dépend (§ 3.4) | un accord qui ne reposerait que sur des sources liées |
| Des lectures du droit qui divergent | elles sont écrites dans la fiche, pas tranchées dans le code | un exemple officiel qui contredit la lecture retenue |
| Un tableau de bord qui rassure à tort | il est fabriqué, jamais écrit à la main ; son dénominateur est la loi (§ 6.6) ; un changement d'état se lit dans le diff | un chiffre du tableau qu'une source contredit |

---

## Annexe A — Deux fiches d'exemple : les trimestres des enfants

Écrites à partir de ce que le dépôt contient déjà — `majoration_duree_assurance.csv`
et son en-tête, les entrées de `veille.yaml` et de `frontiere_contributive.yaml`,
le code et les tests — et des rédactions d'articles lues mot à mot dans
l'index LEGI. Les bornes sont des intervalles [début, fin) ; `null` veut dire
« sans borne ». Celles que le modèle donne à l'année restent à relire au jour
près. Elles servent de gabarit : une fiche cite ses textes par identifiant,
écrit ses bornes en intervalles, jamais en prose, et range ses questions
ouvertes dans `sources`, où une vue les lit.
...
- **La liste des textes trouve ce que la mémoire avait laissé.** Chaque
  ...
  comme restant à rattacher. Pour le régime général, cinq rédactions qui
  changent le droit attendent encore, et le décret de 2003 accordait les
  trimestres un par un, à chaque anniversaire de l'enfant. La fiche le
  montre au lieu de le laisser dormir.
