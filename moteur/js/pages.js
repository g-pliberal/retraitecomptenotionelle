...
export class Saisie {
  ...
  verifier() {
    ...
    if (!(this.annee_revenu >= ANNEE_MINIMALE
          && this.annee_revenu <= ANNEE_MAXIMALE)) {
      throw new ErreurSaisie(
        `Année des montants saisis attendue entre ${ANNEE_MINIMALE} et `
        + `${ANNEE_MAXIMALE} (reçu : ${this.annee_revenu}).`,
      );
    }
    this.verifierUnite();
   ...
   *
   * L'année, elle, se contrôle en dehors de cette méthode : elle date aussi
   * les repères — salaire moyen, SMIC, plafond — que la page affiche sous le
   * champ, et ceux-là paraissent quelle que soit l'unité choisie. Le contrôle
   * vivait ici, derrière le retour anticipé ci-dessous, si bien qu'en
   * multiples du salaire moyen « ?annee_revenu=9999 » passait et faisait
   * afficher un SMIC de 10 932 € par mois.
   */
  verifierUnite() {
...
function choixUnite(saisie) {
  return `<div class="grille">${
    ...
    g.champ("annee_revenu", "…de l'année", saisie.annee_revenu,
      ...
      + "ci-dessous", "number",
      { min: String(ANNEE_MINIMALE), max: String(ANNEE_MAXIMALE), step: "1" })
  }</div>`;
}
...
remplacement</strong>, qui rapporte la pension annuelle au dernier revenu
d'activité ramené à l'année pleine, compare donc un brut à un brut, et il est
plus bas qu'un taux calculé sur des nets, la pension étant moins prélevée que le
salaire. Ses deux termes étant pris dans les euros de leur propre année, il ne
dépend pas de l'unité d'affichage. Le chiffre mis en avant, lui, est en euros
constants de ${saisie.euros}, c'est-à-dire au pouvoir d'achat de ${saisie.euros} :
seule unité qui permette de comparer des liquidations d'années différentes.
...
function cascade(comparaison, saisie) {
  ...
  const renvoiCascade = liquidation !== saisie.euros
