...
const DEFAUTS = Object.freeze({
  ...
  // scénario 6. Le défaut est la personne seule, comme pour l'ASPA.
  foyer: "seul",
...
export class Contexte {
  ...
  /** Le coût agrégé des six systèmes — une seconde de calcul, une fois. */
  cout() {
    if (!this._cout) {
...
export function rendre(contexte, chemin, parametres = null) {
  ...
  try {
    ...
    return [TITRES["/"], presentation(saisie) + messageErreur(erreur.message)
      + formulaire(saisie, contexte)];
  }
...
function basculeUnite(saisie, echelle) {
  ...
  return '<p class="discret" style="margin:0.9rem 0 0">'
    + `<a href="#/?${echapper(saisie.requete(remplacements))}">${libelle}</a></p>`;
}
...
function titresScenarios(saisie) {
  return [
    ...
    ["notionnel_liberal", `6. Libéral : 18 % dès ${saisie.bascule}, garantie`],
  ];
}
...
<div class="scenario">
  ...
  const scenarios = bloc("actuel", "1. Système actuel",
    ...
    + bloc("liberal",
      `6. Proposition libérale : 18 % pour tous dès ${saisie.bascule}, `
      + "garantie vieillesse",
      `le scénario 4 jusqu'à ${saisie.bascule}, 18 % pour tous ensuite, `
...
function garantieVieillesse(comparaison, saisie) {
  ...
  const lignes = [
    ...
    ["d) Pension contributive",
      `le compte notionnel du scénario 6 — taux réels avant ${bascule}, `
...
${g.tableau(
    ...
    lignes,
    ["", "", "nombre"],
    `La garantie vieillesse du scénario 6, étape par étape, en euros de ${annee}`,
    true,
  )}
${lecture}
...
douze : il l'estime à ${milliards(c.cumul(COMPOSANTE_GARANTIE), 0)} sur
...
<ul class="serree">
  ...
  pèse ${g.pourcentage(c.poids.agent_sncf_conduite, false, 1)} et non
  ${g.pourcentage(1 / CAS_TYPES.length, false, 1)}. Ce qu'un effectif de caisse
...
calculer le salaire de référence. Il vaut <strong>×${g.nombre(revalPratiquee, 0)}</strong> sur la période, soit près de cinq
...
+5,2 points pour la génération 1920, +0,0 pour 1945, et -0,4 pour 1958, dont la
